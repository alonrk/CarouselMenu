...
//Rare exceptions to indicate a bug in the _carouselmenu code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_carouselmenu = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_carouselmenu]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.carouselmenu.module"
